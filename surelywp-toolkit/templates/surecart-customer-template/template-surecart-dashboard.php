<?php
/**
 * Template Name: Surelywp Surecart Customer Dashboard
 *
...
<body <?php body_class( is_user_logged_in() ? 'has-columns' : '' ); ?>>
	...
	<?php else : ?>
		<div class="sc-dashboard">
			<div class="sc-dashboard__column dashboard-left is-sticky">
				...
				<div class="sc-dashboard__back sc-pin-bottom">
					<sc-button href="<?php echo esc_url( apply_filters( 'sc_customer_dashboard_back_home_url', get_home_url() ) ); ?>" type="text" class="sc-link-home" aria-label="back home" role="button">
						...
						<?php echo esc_html( apply_filters( 'sc_customer_dashboard_back_home_text', __( 'Back Home', 'surelywp-toolkit' ) ) ); ?>
					</sc-button>
				</div>
				...
				<div class="sc-dashboard__user-menu">
					<sc-dropdown style="width: 100%;" placement="top-right" distance="20" role="select" aria-multiselectable="false">
						<sc-flex class="sc-dashboard__menu-trigger" align-items="center" justify-content="space-between" slot="trigger" tabindex="0" style="<?php echo isset( $attributes['color'] ) ? 'color:' . esc_attr( $attributes['color'] ) . ';' : ''; ?>" aria-label="<?php echo esc_attr__( 'Open or close account menu', 'surelywp-toolkit' ); ?>" role="button">
						...
						<sc-menu tabindex="0">
							...
							<?php if ( ! empty( $data['logout_link'] ) ) : ?>
								<sc-menu-item href="<?php echo esc_url( $data['logout_link'] ); ?>"  role="option">
									...
									<?php echo esc_html_e( 'Log Out', 'surelywp-toolkit' ); ?>
								</sc-menu-item>
							<?php endif; ?>
						</sc-menu>
					</sc-dropdown>
			...
			<?php
			if ( ! empty( $custom_tabs ) && in_array( $active_tab, $custom_tabs, true ) ) {
