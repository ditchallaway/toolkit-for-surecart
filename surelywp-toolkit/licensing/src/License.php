...
class License {
	...
	public function get_current_release( $expires_in = 900 ) {
		...
		if ( empty( $key ) ) {
			return new \WP_Error( 'license_key_missing', esc_html( $this->client->__( 'Please enter a license key.' ) ) );
		}
	...
	public function validate_license( $license ) {
		if ( is_wp_error( $license ) ) {
			if ( $license->get_error_code( 'not_found' ) ) {
				return new \WP_Error( $license->get_error_code(), esc_html( $this->client->__( 'This license key is not valid. Please double check it and try again.' ) ) );
			}
			return $license;
		}
