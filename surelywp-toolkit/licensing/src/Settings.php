...
class Settings {
	...
	public function settings_output() {
		...
		$content_class = apply_filters( 'surelywp_admin_panel_content_class', 'surelywp-admin-panel-content-wrap', $option_key );
		?>
		<div class="wrap">
			<?php settings_errors(); ?>
				<div class="<?php echo esc_attr( $content_class ); ?>">
				<form method="post" action="<?php echo esc_attr( $this->form_action_url() ); ?>">
					<div class="surelywp-body-content-header <?php echo esc_attr( $content_class ); ?>">
							...
							<tr class="surelywp-field-label" >
								<td>
									<div class="<?php echo esc_attr( $this->name ) . '-form-container'; ?>">
										...
										$dashboard_url     = $dashboard_page_id ? get_permalink( $dashboard_page_id ) : home_url( '/customer-dashboard' );
										...
										echo wp_kses_post(
											sprintf(
												$this->client->__(
													'Enter the license from your <a href="%s">customer dashboard</a> to activate the %s For SureCart plugin.',
													'surecart'
												),
												esc_url( $dashboard_url ),
												esc_html( $this->menu_args['plugin_name'] )
											)
										);
										?>
										</label>
										...
										<div class="licence-submit-btn <?php echo 'activate' === $action ? 'active' : 'deactive'; ?>">
											<?php submit_button( 'activate' === $action ? $this->client->__( 'Activate', 'surelywp-fw' ) : $this->client->__( 'Deactivate', 'surelywp-fw' ) ); ?>
										</div>
	...
	public function license_form_submit() {
		...
		if ( isset( $_GET['active'] ) ) {
			$this->add_success( 'activated', $this->client->__( 'Your license has been activated for this site!', 'surecart' ) );
		}
		...
		if ( 'activate' === $_POST['_action'] ) {
			...
			if ( ! empty( $this->menu_args['activated_redirect'] ) ) {
				$this->redirect( $this->menu_args['activated_redirect'] . '&active=true' );
				exit;
			}
