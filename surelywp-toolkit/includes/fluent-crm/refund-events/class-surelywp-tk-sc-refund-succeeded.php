...
if ( ! class_exists( 'Surelywp_Tk_Sc_Refund_Succeeded' ) ) {
	 ...
	 * @since   1.0.0
	 */
	class Surelywp_Tk_Sc_Refund_Succeeded extends BaseTrigger {
		...
		public function __construct() {
			...
			$this->triggerName = 'surecart/refund_succeeded';
		...
		public function getTrigger() {
			return array(
				...
				'label'       => esc_html__( 'SureCart - Refund Succeeded', 'surelywp-toolkit' ),
				'description' => esc_html__( 'Occurs when a refund\'s status changes to succeeded', 'surelywp-toolkit' ),
		...
		public function getSettingsFields( $funnel ) {
			...
			return array(
				'title'     => esc_html__( 'SureCart - Refund Succeeded', 'surelywp-toolkit' ),
				'sub_title' => esc_html__( 'Occurs when a refund\'s status changes to succeeded', 'surelywp-toolkit' ),
				'fields'    => array(
		...
		public function handle( $funnel, $original_args ) {
			...
			$refund_obj  = $original_args[0];
			$customer_id = $refund_obj->customer ?? '';
			...
			if ( ! $this->isProcessable( $funnel, $refund_obj, $subscriber_data ) ) {
				return false;
			}
			...
			( new FunnelProcessor() )->startFunnelSequence(
				...
				array(
					...
					'notes'               => wp_json_encode( $refund_obj ),
				)
			);
		}
		 ...
		 *
		 * @param object $funnel The funnel object.
		 * @param object $refund_obj The object of the arguments that are passed to the callback.
		 * @param object $subscriber_data The subsscriber data.
		 *
		 * @package Toolkit For SureCart
		 * @since   1.0.0
		 */
		private function isProcessable( $funnel, $refund_obj, $subscriber_data ) {
			...
			$refund_obj_expand = Refund::with( array( 'refund_items', 'refund_items.line_item', 'line_item.price' ) )->find( $refund_id );

			$product_ids = array();
			if ( isset( $refund_obj_expand->data ) && ! empty( $refund_obj_expand->data ) ) {
				foreach ( $refund_obj_expand->data as $item ) {
