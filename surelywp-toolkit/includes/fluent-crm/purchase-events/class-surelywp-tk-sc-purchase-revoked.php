<?php
/**
 * Surecart Purchase Revoked Trigger Integration
 *
...
if ( ! class_exists( 'Surelywp_Tk_Sc_Purchase_Revoked' ) ) {
	 ...
	 * @since   1.0.0
	 */
	class Surelywp_Tk_Sc_Purchase_Revoked extends BaseTrigger {
		...
		public function __construct() {
			...
			$this->triggerName = 'surecart/purchase_revoked';
		...
		public function getTrigger() {
			return array(
				...
				'label'       => esc_html__( 'SureCart - Purchase Revoked', 'surelywp-toolkit' ),
				'description' => esc_html__( 'This can happen if an order\'s any product is refunded or if a subscription is canceled or expired.', 'surelywp-toolkit' ),
		...
		public function getSettingsFields( $funnel ) {
			...
			return array(
				'title'     => esc_html__( 'Occurs whenever a purchase is revoked', 'surelywp-toolkit' ),
				'sub_title' => esc_html__( 'This can happen if an order\'s any product is refunded or if a subscription is canceled or expired.', 'surelywp-toolkit' ),
				'fields'    => array(
		...
		public function handle( $funnel, $original_args ) {
			...
			$sc_purchase_obj = $original_args[0];
			$customer_id     = $sc_purchase_obj->customer ?? '';
			...
			if ( ! $this->isProcessable( $funnel, $sc_purchase_obj, $subscriber_data ) ) {
				return false;
			}
			...
			( new FunnelProcessor() )->startFunnelSequence(
				...
				array(
					...
					'notes'               => wp_json_encode( $sc_purchase_obj ),
				)
			);
		}
		 ...
		 *
		 * @param object $funnel The funnel object.
		 * @param object $sc_purchase_obj The object of the arguments that are passed to the callback.
		 * @param object $subscriber_data The subsscriber data.
		 *
		 * @package Toolkit For SureCart
		 * @since   1.0.0
		 */
		private function isProcessable( $funnel, $sc_purchase_obj, $subscriber_data ) {
			...
			// Get the product id.
			$product_id = '';
			$product    = $sc_purchase_obj->product;
