<?php
/**
 * Surecart Order Partially Fulfilled.
 *
...
if ( ! class_exists( 'Surelywp_Tk_Sc_Order_Partially_Fulfilled' ) ) {
	 ...
	 * @since   1.0.0
	 */
	class Surelywp_Tk_Sc_Order_Partially_Fulfilled extends BaseTrigger {
		...
		public function __construct() {
			...
			$this->triggerName = 'surecart/order_partially_fulfilled';
		...
		public function getTrigger() {
			return array(
				...
				'label'       => esc_html__( 'SureCart - Order Partially Fulfilled', 'surelywp-toolkit' ),
				'description' => esc_html__( 'Occurs when an order\'s fulfillment status changes to partially fulfilled', 'surelywp-toolkit' ),
		...
		public function getSettingsFields( $funnel ) {
			...
			return array(
				'title'     => esc_html__( 'SureCart - Order Partially Fulfilled', 'surelywp-toolkit' ),
				'sub_title' => esc_html__( 'Occurs when an order\'s fulfillment status changes to partially fulfilled', 'surelywp-toolkit' ),
				'fields'    => array(
		...
		public function handle( $funnel, $original_args ) {
			...
			$sc_order_obj = $original_args[0];
			$checkout_id  = $sc_order_obj->checkout;
			$checkout_obj = Checkout::with( array( 'purchases' ) )->find( $checkout_id );

			$customer_id = $checkout_obj->customer ?? '';
			...
			if ( ! $this->isProcessable( $funnel, $checkout_obj, $subscriber_data ) ) {
				return false;
			}
			...
			( new FunnelProcessor() )->startFunnelSequence(
				...
				array(
					...
					'notes'               => wp_json_encode( $checkout_obj ),
				)
			);
		}
		 ...
		 *
		 * @param object $funnel The funnel object.
		 * @param object $sc_checkout_obj The object of the arguments that are passed to the callback.
		 * @param object $subscriber_data The subsscriber data.
		 *
		 * @package Toolkit For SureCart
		 * @since   1.0.0
		 */
		private function isProcessable( $funnel, $sc_checkout_obj, $subscriber_data ) {
			...
			// Get the product id.
			$product_ids = array();
			$purchases   = $sc_checkout_obj->purchases;
			if ( isset( $purchases->data ) && ! empty( $purchases->data ) ) {
				foreach ( $purchases->data as $purchase ) {
					$product_ids[] = $purchase->product;
				}
			}

			// check the products ids.
			if ( $conditions['product_ids'] && ! array_intersect( $product_ids, $conditions['product_ids'] ) ) {
				return false;
			}
